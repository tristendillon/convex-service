...
// This file is based on the implementation from https://github.com/get-convex/convex-helpers/pull/701 by SamJB123.
// We do not use the version of convex-helpers that includes this Zod v4 integration, so we use the file as written in that PR.
// Please refer to the PR for original authorship and context.

// This will stay as a copy unless the PR is merged to the convex-helpers repo. in that instance we will use the version from the repo.
 ...
 */
export function createZidClass<TableName extends string>(
  tableName: TableName
): ZidClass<TableName> {
  ...
  return {
    issues: error.issues.map((issue) => ({
      ...
      ...(issue.code === 'invalid_format' &&
        'format' in issue && { format: String(issue.format) }),
...
export function transformZodOutputToConvex(
  data: any,
  zodValidators: Record<string, z.ZodType>
): any {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (zodValidator instanceof z.ZodTuple && Array.isArray(value)) {
      ...
      zodValidator instanceof z.ZodObject &&
      typeof value === 'object' &&
      value !== null
    ) {
  ...
  function transformValue(value: any, zodSchema: z.ZodType): any {
    ...
    if (zodSchema instanceof z.ZodOptional) {
      return value === undefined
        ? undefined
        : transformValue(value, zodSchema.unwrap() as z.ZodType)
    }
    ...
    if (zodSchema instanceof z.ZodArray && Array.isArray(value)) {
      return value.map((item) =>
        transformValue(item, zodSchema.element as z.ZodType)
      )
    }
...
function createTupleAcceptingValidator(zodSchema: z.ZodType): any {
  ...
  if (zodSchema instanceof z.ZodOptional) {
    return v.optional(
      createTupleAcceptingValidator(zodSchema.unwrap() as z.ZodType)
    )
  }
...
function preTransformClientArgs(
  args: any,
  zodValidators: Record<string, z.ZodType>
): any {
...
function applyForwardTransformsToValue(value: any, schema: z.ZodType): any {
  ...
  const transformMetadata =
    transformRegistry.getTransformForSchema(actualSchema)
  ...
  if (actualSchema instanceof z.ZodArray && Array.isArray(value)) {
    return value.map((item) =>
      applyForwardTransformsToValue(item, actualSchema.element as any)
    )
  }
...
function applyReverseTransformsToValue(value: any, schema: z.ZodType): any {
  ...
  if (actualSchema instanceof z.ZodArray && Array.isArray(value)) {
    return value.map((item) =>
      applyReverseTransformsToValue(item, actualSchema.element as any)
    )
  }
 ...
 */
export function transformConvexDataToZod(
  data: any,
  originalZodSchema: z.ZodType
): any {
...
function customFnBuilder(builder: any, mod: any): any {
  ...
  return (fn: Registration<any, any, any, any>): any => {
    let args = fn.args ?? {}
    ...
    const wrappedBuilder = (fnDef: any) => {
      ...
      if (
        fnDef.handler &&
        fnDef.args &&
        Object.values(originalZodArgs).some((v) => v instanceof z.ZodType)
      ) {
      ...
      if (!hasZodFields && isEmptyObject) {
        ...
        const convexFn = {
          ...
          handler: async (ctx: any, allArgs: any) => {
            ...
            const convexCompatibleArgs = transformZodOutputToConvex(
              parsed.data,
              originalZodArgs
            )
...
>(
  query:
    ...
    | CustomBuilder<any, any, any, any, GenericQueryCtx<DataModel>, Visibility>,
...
> {
  <
    ...
    ReturnValue extends ReturnValueForOptionalZodValidator<ReturnsZodValidator> = any
  >(
    fn:
      | ((ArgsValidator extends EmptyObject
          ...
          handler: (
            ...
            ?
                | z.output<ReturnsZodValidator>
                | Promise<z.output<ReturnsZodValidator>>
            : ReturnsZodValidator extends ZodValidator
            ?
                | z.output<z.ZodObject<ReturnsZodValidator>>
                | Promise<z.output<z.ZodObject<ReturnsZodValidator>>>
            : ReturnsZodValidator extends PropertyValidators
            ?
                | ObjectType<ReturnsZodValidator>
                | Promise<ObjectType<ReturnsZodValidator>>
            : any
    ...
    ArgsArrayToObject<
      ArgsValidator extends ZodValidator
        ? [
            ObjectType<
              ConvexValidatorFromZodFields<ArgsValidator, 'required'>
            > &
...
export type ReturnValueForOptionalZodValidator<
  ReturnsValidator extends z.ZodType | ZodValidator | PropertyValidators
...
export function convexToZod<V extends GenericValidator>(
  ...
  switch (convexValidator.kind) {
    ...
    case 'object': {
      ...
      if ((objectValidator as any)._zodTuple) {
        ...
        if (isSequential) {
          ...
          const items = sortedKeys.map((idx) => convexToZod(fields[`_${idx}`]))
        ...
        if (allNumeric) {
          ...
          if (isSequential) {
            ...
            const items = sortedNumeric.map((idx) =>
    ...
    case 'record': {
      ...
      // Always convert record values normally - don't try to be smart about unions
  ...
  // For VOptional validators, create a union with null instead of using .optional()
  // This matches the expected behavior where optional fields should become unions with null
  if (isOptional && convexValidator.kind !== 'union') {
    zodValidator = z.union([zodValidator, z.null()])
  }

  // Check for default metadata
  if (
    convexValidator &&
    typeof convexValidator === 'object' &&
    '_zodDefault' in convexValidator
  ) {
    const defaultValue = (convexValidator as any)._zodDefault
    if (
      zodValidator &&
      typeof zodValidator === 'object' &&
      'default' in zodValidator &&
      typeof zodValidator.default === 'function'
    ) {
...
type ZodTypeFromConvexValidator<V extends GenericValidator> = V extends VString<
  any,
  any
>
  ? z.ZodString
  ...
  : V extends VNull<any>
  ? z.ZodNull
  : V extends VAny<any, any>
  ? z.ZodAny
  : V extends VBytes<any, any>
  ? z.ZodBase64
  : V extends VId<any, any>
  ? z.ZodPipe<z.ZodPipe<z.ZodString, any>, any> // zid returns a complex branded type
  : V extends VLiteral<infer T, any>
  ? z.ZodType<T> // Use generic ZodType instead of ZodLiteral to avoid constraint issues
  : V extends VArray<any, infer E, any>
  ? z.ZodArray<z.ZodType<Infer<E>>>
  : V extends VObject<any, infer F, any, any>
  ? z.ZodObject<{
      [K in keyof F]: z.ZodType<Infer<F[K]>>
    }>
  : V extends VRecord<any, any, infer Val, any, any>
  ? z.ZodRecord<z.ZodString, ZodTypeFromConvexValidator<Val>>
  ...
  ? Members extends readonly [
    ...
    ? Rest extends readonly GenericValidator[]
      ? z.ZodUnion<
          [
            ZodTypeFromConvexValidator<A>,
            ZodTypeFromConvexValidator<B>,
            ...{
              [I in keyof Rest]: ZodTypeFromConvexValidator<
                Rest[I] & GenericValidator
              >
            }
          ]
        >
      : z.ZodUnion<
          [ZodTypeFromConvexValidator<A>, ZodTypeFromConvexValidator<B>]
        >
    : z.ZodUnion<[z.ZodAny, z.ZodAny]>
  : V extends VOptional<infer Inner>
  ? z.ZodOptional<ZodTypeFromConvexValidator<Inner>>
  : z.ZodType<Infer<V>>
...
function convertZodTupleToConvex(
  actualValidator: z.ZodTuple,
  useRecursiveCall: boolean = false
): GenericValidator {
  ...
  // Convert all tuple items to their Convex validators
  const convexItems = items.map((item) =>
...
function convertZodReadonlyToConvex(
  actualValidator: z.ZodReadonly
): GenericValidator {
  ...
  if (isZid(actualValidator)) {
    ...
    switch (defType) {
      ...
      case 'pipe': {
        ...
        const transformMetadata =
          transformRegistry.getTransformForSchema(actualValidator)
        if (transformMetadata) {
        ...
        // Check for custom branded validators
          ...
          metadata?.isBrandedValidator &&
          metadata?.convexValidatorFactory
        ) {
...
function zodOutputToConvexInternal<Z extends z.ZodType>(
  ...
  if (zodValidator instanceof z.ZodTransform) {
    ...
    if (metadata?.brand && metadata?.originalSchema) {
      ...
      return zodToConvexInternal(
        metadata.originalSchema
      ) as ConvexValidatorFromZodOutput<Z, 'required'>
    }
  ...
  return zodToConvexInternal(zodValidator) as ConvexValidatorFromZodOutput<
    Z,
    'required'
  >
}
...
const customValidatorRegistry = new Map<
  string,
  {
...
export function zTemplate(
  ...
  if (Array.isArray(partsOrStrings) && 'raw' in partsOrStrings) {
    ...
    for (let i = 0; i < strings.length; i++) {
      ...
      if (i < schemas.length)
        parts.push(schemas[i] as z.core.$ZodTemplateLiteralPart)
    }
...
export function createBidirectionalSchema<T extends Record<string, z.ZodType>>(
  ...
  pick: <K extends keyof T>(
    ...
    zod: Pick<T, K>
    convex: Pick<
      { [P in keyof T]: ConvexValidatorFromZod<T[P], 'required'> },
      K
    >
  }
  ...
  const convexSchemas = {} as {
    [K in keyof T]: ConvexValidatorFromZod<T[K], 'required'>
  }

  for (const [key, zodSchema] of Object.entries(schemas)) {
    convexSchemas[key as keyof T] = zodToConvex(
      zodSchema as z.ZodType
    ) as ConvexValidatorFromZod<T[keyof T], 'required'>
  }
  ...
  return {
    ...
    extend: (extension) =>
      createBidirectionalSchema({ ...schemas, ...extension }),
  }
}
...
export const convexZodTestUtils = {
  ...
  testValueConsistency: <T>(
    ...
    const results = {
      ...
      errors: [] as Array<{
        type:
          | 'valid_value_failed_zod'
          | 'invalid_value_passed_zod'
          | 'conversion_error'
        value: any
        error?: any
        details?: string
    ...
    for (const value of testValues.valid) {
      ...
      if (!zodResult.success) {
        ...
        if (options?.verbose) {
          console.warn('Valid value failed Zod validation:', {
            value,
            error: zodResult.error,
          })
        }
  ...
  generateTestData: (schema: z.ZodType): any => {
    ...
    if (schema instanceof z.ZodArray) {
      const elementData = convexZodTestUtils.generateTestData(
    ...
    if (schema instanceof z.ZodOptional) {
      ...
      return Math.random() > 0.5
        ? undefined
        : convexZodTestUtils.generateTestData(schema.unwrap() as z.ZodType)
    }
    ...
    if (schema instanceof z.ZodRecord) {
      return {
        key1: convexZodTestUtils.generateTestData(
          schema.valueType as z.ZodType
        ),
        key2: convexZodTestUtils.generateTestData(
          schema.valueType as z.ZodType
        ),
      }
    }
      ...
      return {
        ...
        error: success
          ...
          : {
              ...
              roundTrip: roundTripResult.success
                ? 'passed'
  ...
  validateBidirectionalSchemas: <T extends Record<string, z.ZodType>>(
    ...
    for (const key of schemas.keys()) {
      ...
      const testValue =
        testData?.[key] ?? convexZodTestUtils.generateTestData(zodSchema)
